"""
Phase 2 deliberation engine: one closed M15 bar in, one recorded CycleResult out.
    ...
    snapshot -> MarketContext (bars, features, session, calendar)
             ...
             -> tier 1 panel (rules; other backends: `backend_not_built`)
             -> protocol.resolve -> exits + sizing -> ShadowIntent (never published)

Shadow only: nothing here can publish an intent. The engine never raises: any
error becomes status ERROR (APP-V6-ERROR) with the error type in `hold_detail`,
keeping whatever the cycle had gathered. A decision after bar close +
V6_DECISION_DEADLINE_S is LATE. With a FakeClock the engine is deterministic.
"""
...
from ..providers.base import PROVIDER_STATUS_FAILED, RULES_PROVIDER_NAME, AgentProvider
...
class EngineDeps:
    ...
    panel: AgentProvider | None       # None: the configured backend is not built
...
def panel_for_backend(settings: V6Settings, rules: OfflineProvider) -> AgentProvider | None:
    """Phase 2 builds only the rules backend."""
...
def build_engine(settings: V6Settings, clock: Clock, bars: BarReader,
                 breakers: BreakerSource) -> "DeliberationEngine":
...
class DeliberationEngine:
    ...
    def _deadline(self, request: CycleRequest) -> float:
        return float(as_of_for(request.snapshot) + self._deps.settings.decision_deadline_s)
    ...
    async def _decide(self, draft: CycleDraft, tier0: Tier0) -> CycleOutcome:
        ...
        if self._late(request):
            return self._hold(draft, HoldReason.LATE, DETAIL_LATE, STATUS_LATE)
        panel = await self._tier1(tier0, request)
        deliberation_ms = elapsed_ms(draft.started_at, self._now()) - draft.tier0_ms
        if panel is None:
            return self._hold(draft.update(deliberation_ms=max(0, deliberation_ms)),
                              HoldReason.LATE, DETAIL_LATE, STATUS_LATE)
        draft = draft.update(
            ...
            deliberation_ms=max(0, deliberation_ms),
            candidates=label_verdicts(pool, price_action=panel.views.price_action,
                                      decision=panel.decision))
        return self._resolve(draft, tier0, panel)
    ...
    def _resolve(self, draft: CycleDraft, tier0: Tier0, panel: PanelResult) -> CycleOutcome:
        settings = self._deps.settings
        protocol = resolve(ProtocolInput(
            gates=tier0.gates, calendar=tier0.context.calendar,
            offered_ids=tier0.pool.offered_ids, views=panel.views, decision=panel.decision,
        ...
        return self._enter(draft, tier0, panel, protocol)

    def _enter(self, draft: CycleDraft, tier0: Tier0, panel: PanelResult,
               protocol: ProtocolDecision) -> CycleOutcome:
        ...
        if self._late(draft.request):
            return self._hold(draft, HoldReason.LATE, DETAIL_LATE, STATUS_LATE)
        return draft.update(shadow_intent=order.intent).finish(
            STATUS_ENTER, None, "", self._now())
