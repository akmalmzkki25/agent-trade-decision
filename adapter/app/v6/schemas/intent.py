...
INTENT_SCHEMA: Final[str] = "v6.intent.1"
...
ORDER_TYPES_BY_SIDE: Final[Mapping[str, frozenset[str]]] = MappingProxyType({
    "buy": frozenset({"BUY_LIMIT", "BUY"}), "sell": frozenset({"SELL_LIMIT", "SELL"})})
...
class PollResponse(_Strict):
    ...
    def _consistent(self) -> "PollResponse":
        problems = _intent_problems(self) if self.has_intent else _leftover_fields(self)
        if problems:
            raise ValueError("; ".join(problems))
        return self


def _leftover_fields(response: PollResponse) -> list[str]:
    defaults = PollResponse.model_fields
    leftover = [name for name in INTENT_FIELDS
                if getattr(response, name) != defaults[name].default]
    return [f"has_intent=false must leave {', '.join(leftover)} empty"] if leftover else []
...
def order_problems(*, side: str, order_type: str, entry: float, sl: float, tp: float,
    ...
    checks = (
        ...
        (not is_limit or pending_expiry_epoch >= valid_until_epoch + MIN_PENDING_LIFETIME_S,
         f"a limit order must expire at least {MIN_PENDING_LIFETIME_S} s after valid_until"),
        (is_limit or pending_expiry_epoch == 0, "a market order has no pending expiry"),
    )
...
def _intent_problems(r: PollResponse) -> list[str]:
    ...
    geometry = order_problems(
        ...
        time_barrier_s=r.time_barrier_s)
